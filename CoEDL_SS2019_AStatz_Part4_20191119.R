...
table(data$SUFlike); mean(data$SUFlike); sd(data$SUFlike)
...
p1d <- data %>%
  ...
  dplyr::mutate(Mean=round(mean(SUFlike), 1)) %>%
  dplyr::mutate(SD=round(sd(SUFlike),1))
# start plot
ggplot(p1d, aes(Gender, SUFlike)) +
...
# create data for plotting: ConversationType
p3d <- data %>%
  dplyr::select(ID, SUFlike, ConversationType) %>%
  dplyr::mutate(SUFlike = SUFlike*100) %>%
  group_by(ID, ConversationType) %>%
  dplyr::summarize(SUFlike=mean(SUFlike))
# start plot
ggplot(p3d, aes(ConversationType, SUFlike)) +
  geom_boxplot(width=0.5, fill="gray80", color = "gray20") +
  scale_fill_manual(values=rep("grey90", 2)) + 
  theme_set(theme_bw(base_size = 10)) +
  ylim(-5, 100) +
...
p7d <- data %>%
  dplyr::select(ID, SUFlike, Gender, ConversationType) %>%
  dplyr::mutate(SUFlike = SUFlike*100) %>%
  group_by(ID, Gender, ConversationType) %>%
  dplyr::summarize(SUFlike=mean(SUFlike))
# start plot
